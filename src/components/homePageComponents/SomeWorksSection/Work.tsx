    ...
    <div className="work__slide row">
      <div className="work__gallery col s12 l12">
        <div className="work__big">
          <Zoom >
            {work.img && <img className="responsive-img" src={work.img.path} alt="" />}
          </Zoom>
        </div>
          ...
          {work.files?.length &&
            work.files.map((file, index) => {
              return (
                <a
                  key={index}
                  aria-label={index.toString() + " фото"}
                  data-src={file.path}
                  className=""
                >
                  <Zoom>
                    {file.path && <img className="responsive-img" src={file.path} alt="" />}
                  </Zoom>
                </a>
              );
            })}
      ...
      <div className="work__description col s12 l12">
        ...
        <div className="work__link">
          <Link
            className="waves-effect waves-light orange btn-large"
            to={`/post/${work._id}`}
          >
            Перейти
          </Link>
        ...
        {state.user?.status === "admin" && (
          <Link
            className="waves-effect waves-light orange btn" style={{position: 'absolute', top: 0, right: 0}}
